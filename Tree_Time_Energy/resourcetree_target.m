function [coalitionM,coalitionC,liliT,liliE] = resourcetree_target(coalitionMembers,coalitionCost,uav,target,attack_target)
...
[coalition,liliT,liliE] = compute_target_fun2(finalCoalition,coalitionMembers,coalitionCost,uav,target(attack_target).location);
coalitionM = [];    %最终选出的联盟
coalitionC = [];    %对应的时间
...
function [coalition,liliT,liliE] = compute_target_fun2(finalCoalition,coalitionMembers,coalitionCost,uav,targetLocation)  
%finalCoalition中的每一行为一个满足目标资源需求的联盟，用序号表示
%变化目标函数：max(EAT)+sum(E),其中max(EAT)转化为其对应的无人机的能耗
    ...
    for i=1:groupNum                        %计算每一行的目标值
        ...
        groupFunValues(i) = 0.4*tempValue(pos) + 0.6*sum(tempValue);    %计算目标函数值:max(EAT)+sum(E)，如果需要加权重，则在此行代码处添加
    end
    [~,pos] = min(groupFunValues);
    coalition = finalCoalition(pos,:);
    %2020/6/18 added
        tempValue = zeros(1,size(coalition,2));  %存放组合中每个无人机的能耗
        for j=1:size(coalition,2)                %计算组合中每个无人机的能耗
            if coalitionC(coalition(j))~=0
                tempValue(j) = coalitionC(coalition(j));
            else
                tempValue(j) = compute_energy(uav(coalitionMembers(coalition(j))),targetLocation);
                coalitionC(coalition(j)) = tempValue(j);
            end
        end
        tempCost = [];              %记录组合中每个无人机的时间EAT
        for k=1:size(coalition,2)
            tempCost = [tempCost coalitionCost(coalition(k))];
        end
        liliT = max(tempCost);
        liliE = sum(tempValue);
end
