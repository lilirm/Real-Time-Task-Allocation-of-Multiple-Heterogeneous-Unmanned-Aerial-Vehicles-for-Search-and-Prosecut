function [uav,liliE] = dubins_path_plan_energy(uav,target,R)  %计算路径点坐标,并放入planning_route属性中
...
[uav,liliE] = compute_energy_consume(uav,hudu,tangent_site,R,target.location);
...
function [uav,liliE] = compute_energy_consume(uav,hudu,tangent_site,R,targetLocation)      %根据dubins曲线计算这段路径上的能耗
%分两段计算能耗：匀速圆周段+匀速直线段
