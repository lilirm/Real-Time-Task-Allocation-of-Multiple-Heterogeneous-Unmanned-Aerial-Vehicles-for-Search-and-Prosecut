function [UAV_task] = clashavoid1(find_targets,uav,target)  %解决死锁情况
% 1、单无人机侦察到多个目标
% 2、多无人机侦察到同一目标
...
for ft = 1:length(find_targets)
  ...
  distance = zeros(1,size(find_target,2));        %无人机uavId到各目标点的时间
  for ds = 1:length(find_target)
      distance(ds) = compute_EAT(uav(uavId),target(find_target(ds)).location,uav(uavId).turnRadius);
  end
  
  if length(find_target)>1
      [distance,sortOrder] = sort(distance);   %按照时间大小排序，从近到远
...
% disp("final_target:");
% disp(final_target);
while length(final_target)-length(unique(final_target)) >0  % >0表示有重复分配给不同无人机的同一个目标值，=0表示无重复目标
    flag2=0;
...
for i=1:length(repeat)
    ...
    if i~=pos
        ...
        if isempty(find_targets(repeat(i)).detect_target)
            UAV_task(repeat(i)).target = 0;        %不能设为空[]
            continue
        end
