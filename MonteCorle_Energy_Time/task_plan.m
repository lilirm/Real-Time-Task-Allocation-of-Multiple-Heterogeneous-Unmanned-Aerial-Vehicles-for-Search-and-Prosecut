function [missionTime,energyConsume,flag] = task_plan(uav,target,ur,tr,l,b)
runTime = 10000;                   %仿真时间
dt=0.1;                          %采样间隔
deviation =0.01;                 %误差
flag=0;                          %标志全部任务是否完成
    ...
    for cp = 1:length(UAV_task)
        ...
        if UAV_task(cp).needcoalition == 0    %需要组建联盟
            ...
            if candidate_resource-target(attack_target).resource >= 0
                %满足目标资源需求，组建联盟
                [coalition] = formcoalition4(uav,target,attack_target,target_candidate);  % added 2020/5/26
                [uav,target] = path_plan2(attack_target,coalition,uav,target,l);
            else
                continue
            end
        else
