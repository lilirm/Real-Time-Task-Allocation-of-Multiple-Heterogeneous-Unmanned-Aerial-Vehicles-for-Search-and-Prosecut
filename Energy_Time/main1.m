    ...
    for cp = 1:length(UAV_task)
        ...
        if UAV_task(cp).needcoalition == 0    %需要组建联盟
            ...
            if candidate_resource-target(attack_target).resource >= 0
                %满足目标资源需求，组建联盟
                tic
                [coalition,sumE] = formcoalition4(uav,target,attack_target,target_candidate);  % added 2020/5/26
                [uav,target,maxCostT] = path_plan2(attack_target,coalition,uav,target,l);
                toc
            else
                continue
            end
